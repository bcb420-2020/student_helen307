...
```{r}
(expr_dup <- length(expr[which(duplicated(expr[,1]))]))
```
...
* The paper specified their usage of hg19, which is equivalent to GRCh37. [Reference: https://www.ncbi.nlm.nih.gov/assembly/GCF_000001405.13/]
  * __Result__: 700 genes are not mapped
  * __Solution__: used EnsDb.Hsapiens.v75 which is related to GRCh37. [Reference: https://genomicsclass.github.io/book/pages/bioc1_annoCheat.html]
...
(num_dup_id <- length(test_filtered$GENEID[which(duplicated(test_filtered$GENEID))])) # 17 duplicated
...
* __Result__: 12 distinct symbols (41 in total) corresponds to 41 ensembl ids. 
```{r}
dup_gene_name <- (expr_filtered$SYMBOL[which(duplicated(expr_filtered$SYMBOL))])
length(dup_gene_name) # 41
final_dup_gene_name <- unique(expr_filtered$SYMBOL[which(duplicated(expr_filtered$SYMBOL))])
length(final_dup_gene_name) # 12
```
...
```{r}
removed_dup_gene_id <- (expr_filtered$GENEID[which(duplicated(expr_filtered$GENEID))])
length(removed_dup_gene_id) # 0

removed_dup_gene_name <- unique(expr_filtered$SYMBOL[which(duplicated(expr_filtered$SYMBOL))])
length(removed_dup_gene_name) # 0 
```
...
{boxplot(data2plot, 
         ...
         cex.lab = 0.5,
         cex.axis = 0.5, 
         main = "RNASeq Samples before normalization") + 
...
* __Questions__
  ...
  * *Were there expression values that were not unique for specific genes? How did you handle these?*
    * In my case, multiple ensembl ids are mapped to the same gene symbol. The workflow I cited sugguested to map back to ensembl ids and find the highest expression ensembl id as the unique id that can map to the gene name.
  ...
  * *What is the final coverage of your dataset?*
    * The dataset now covers 19815 genes (24 samples) after filteration and normalization.
